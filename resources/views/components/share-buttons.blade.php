@props(['title', 'url' => null, 'showLabel' => true, 'useShortLink' => true])

@php
    $shareUrl = $url ?? url()->current();
@endphp

<div class="flex items-center justify-between flex-wrap gap-4 @if($showLabel) mt-8 pt-6 pb-4 @endif"
     @if($showLabel) style="border-top:1px solid #E4DCC9;" @endif
     x-data="{
        shortUrl: null,
        loading: false,
        async getShortUrl() {
            if (!this.shortUrl && {{ $useShortLink ? 'true' : 'false' }}) {
                this.loading = true;
                try {
                    const response = await fetch('{{ route('short-link.generate') }}', {
                        method: 'POST',
                        headers: {
                    ...
                    this.shortUrl = '{{ $shareUrl }}';
                }
                this.loading = false;
            }
            return this.shortUrl || '{{ $shareUrl }}';
        }
     }">
    @if($showLabel)
    ...
    <div class="flex gap-2">
        ...
        <button x-data="{ copied: false }"
                ...
                :disabled="loading"
                class="w-9 h-9 rounded-full flex items-center justify-center transition hover:opacity-80 relative disabled:opacity-50" style="background:#F3F4F6;color:#374151;"
            ...
            <div x-show="loading" class="w-4 h-4 border-2 border-gray-400 border-t-transparent rounded-full animate-spin"></div>
