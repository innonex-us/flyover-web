<x-admin-layout pageTitle="Pick & Drop Bookings">

    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <form action="{{ route('admin.transfer-bookings.index') }}" method="GET" class="flex flex-col md:flex-row gap-2.5 w-full md:w-auto ml-auto">
            ...
            @if(request()->hasAny(['search', 'status']))
                <a href="{{ route('admin.transfer-bookings.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-600 px-4 py-2 rounded-lg text-sm font-medium transition">Clear</a>
            @endif
        </form>
    </div>
            ...
            <table class="w-full whitespace-nowrap">
                <thead class="border-b border-gray-100">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-gray-400">Route / Type</th>
                        <th class="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-gray-400">Passenger Name</th>
                        <th class="px-5 py-3 text-center text-[10px] font-bold uppercase tracking-widest text-gray-400">Passengers</th>
                        <th class="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-gray-400">Travel Date</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-5 py-3.5">
                            @if($booking->is_custom)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-blue-100 text-blue-700">Custom</span>
                                <div class="text-xs text-gray-400 mt-0.5">{{ Str::limit($booking->pickup_location, 25) }} → {{ Str::limit($booking->drop_location, 25) }}</div>
                            @elseif($booking->route)
                                <div class="text-sm font-medium text-gray-900">{{ $booking->route->name }}</div>
                            @else
                                <span class="text-xs text-red-400">Route deleted</span>
                            @endif
                        </td>
                        <td class="px-5 py-3.5">
                            @if($booking->user)
                        ...
                        <td class="px-5 py-3.5 text-center">
                            <a href="{{ route('admin.transfer-bookings.show', $booking) }}" class="inline-flex items-center text-xs font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition">
                                View
                            </a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="8" class="px-5 py-12 text-center text-sm text-gray-400">No transfer bookings found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
