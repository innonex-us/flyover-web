<x-admin-layout pageTitle="Edit Coupon">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
                <p class="text-2xl font-bold text-gray-900">{{ $coupon->used_count }}</p>
                <p class="text-sm text-gray-500">Times Used</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
                <p class="text-2xl font-bold text-gray-900">{{ $coupon->usages()->distinct('user_id')->count('user_id') }}</p>
                <p class="text-sm text-gray-500">Unique Users</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-200 p-4 text-center">
                <p class="text-2xl font-bold text-red-600">৳{{ number_format($coupon->usages()->sum('discount_amount'), 0) }}</p>
                <p class="text-sm text-gray-500">Total Discount</p>
            </div>
        </div>

        {{-- Form --}}
        <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" id="max_discount_amount" name="max_discount_amount" value="{{ old('max_discount_amount', $coupon->max_discount_amount) }}" min="0" step="0.01"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
            ...
            <div>
                ...
                <select id="applies_to" name="applies_to" required
                    ...
                    <option value="{{ $key }}" {{ old('applies_to', $coupon->applies_to) === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>

            {{-- Specific Items --}}
            <div id="specific-items" style="display: {{ in_array(old('applies_to', $coupon->applies_to), ['tours', 'hotels', 'transfers', 'visas']) ? 'block' : 'none' }};">
                ...
                @php $selectedIds = old('applicable_ids', $coupon->applicable_ids ?? []); @endphp
                
                <div id="tours-select" style="display: {{ old('applies_to', $coupon->applies_to) === 'tours' ? 'block' : 'none' }};">
                    <select name="applicable_ids[]" multiple class="w-full px-3 py-2 border border-gray-300 rounded-lg h-32">
                        @foreach($tours as $tour)
                        <option value="{{ $tour->id }}" {{ in_array($tour->id, $selectedIds) ? 'selected' : '' }}>{{ $tour->title }}</option>
                        @endforeach
                    </select>
                </div>
                
                <div id="hotels-select" style="display: {{ old('applies_to', $coupon->applies_to) === 'hotels' ? 'block' : 'none' }};">
                    <select name="applicable_ids[]" multiple class="w-full px-3 py-2 border border-gray-300 rounded-lg h-32">
                        @foreach($hotels as $hotel)
                        <option value="{{ $hotel->id }}" {{ in_array($hotel->id, $selectedIds) ? 'selected' : '' }}>{{ $hotel->name }}</option>
                        @endforeach
                    </select>
                </div>
                
                <div id="transfers-select" style="display: {{ old('applies_to', $coupon->applies_to) === 'transfers' ? 'block' : 'none' }};">
                    <select name="applicable_ids[]" multiple class="w-full px-3 py-2 border border-gray-300 rounded-lg h-32">
                        @foreach($transfers as $transfer)
                        <option value="{{ $transfer->id }}" {{ in_array($transfer->id, $selectedIds) ? 'selected' : '' }}>{{ $transfer->pickup_location }} → {{ $transfer->drop_location }}</option>
                        @endforeach
                    </select>
                </div>
                
                <div id="visas-select" style="display: {{ old('applies_to', $coupon->applies_to) === 'visas' ? 'block' : 'none' }};">
                    <select name="applicable_ids[]" multiple class="w-full px-3 py-2 border border-gray-300 rounded-lg h-32">
                        @foreach($visas as $visa)
                        <option value="{{ $visa->id }}" {{ in_array($visa->id, $selectedIds) ? 'selected' : '' }}>{{ $visa->country }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" id="min_order_amount" name="min_order_amount" value="{{ old('min_order_amount', $coupon->min_order_amount) }}" min="0" step="0.01"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500">
                </div>
                <div>
