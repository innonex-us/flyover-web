<x-admin-layout pageTitle="Modify Tour Package">
...
<div class="max-w-6xl mx-auto" x-data="formUploader" @submit.prevent="submitForm">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            ...
            <h1 class="text-2xl font-black text-gray-900 tracking-tight">Edit: {{ $package->title }}</h1>
        </div>
    ...
    <form id="package-form" action="{{ route('admin.packages.update', $package) }}" method="POST" enctype="multipart/form-data" class="relative">
        ...
        <div x-show="uploading" x-transition class="absolute inset-0 bg-white/90 backdrop-blur-md z-50 flex flex-col items-center justify-center rounded-2xl" style="display: none;">
            ...
            <p class="text-sm font-black text-gray-900 uppercase tracking-widest">Updating Package: <span x-text="progress + '%'"></span></p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="form-card p-6 sm:p-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $package->title) }}" required class="custom-input text-lg font-bold" placeholder="Enter high-impact title...">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" required class="custom-input" placeholder="Primary tour description...">{{ old('description', $package->description) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="policy" rows="3" class="custom-input text-xs">{{ old('policy', $package->policy) }}</textarea>
                            </div>
                            <div>
                                <label class="input-label">Requirements</label>
                                <textarea name="requirements" rows="3" class="custom-input text-xs">{{ old('requirements', $package->requirements) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="hotel_details" rows="3" class="custom-input text-xs" placeholder="Accommodation details...">{{ old('hotel_details', $package->hotel_details) }}</textarea>
                            </div>
                            <div>
                                <label class="input-label">Additional Information</label>
                                <textarea name="additional_info" rows="3" class="custom-input text-xs" placeholder="Extra notes for travelers...">{{ old('additional_info', $package->additional_info) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea name="travel_tips" rows="3" class="custom-input text-xs" placeholder="Packing tips, local customs...">{{ old('travel_tips', $package->travel_tips) }}</textarea>
                            </div>
                            <div>
                                <label class="input-label">Pickup Note</label>
                                <textarea name="pickup_note" rows="3" class="custom-input text-xs" placeholder="Airport pickup instructions...">{{ old('pickup_note', $package->pickup_note) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                @endphp
                <div class="form-card p-6 sm:p-8" x-data="{ days: {{ json_encode(array_values($itineraryData)) }} }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="form-card p-6" x-data="{ items: {{ json_encode(old('inclusions', $package->inclusions ?? [''])) }} }">
                        ...
                        <div class="space-y-2">
                            <template x-for="(item, index) in items" :key="index">
                                <div class="flex gap-2">
                                    ...
                                    <button type="button" @click="items.splice(index, 1)" x-show="items.length > 0" class="text-gray-300 hover:text-red-500 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                                </div>
                            </template>
                    ...
                    <div class="form-card p-6" x-data="{ items: {{ json_encode(old('exclusions', $package->exclusions ?? [''])) }} }">
            ...
            <div class="space-y-8">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" value="{{ old('price', $package->price) }}" required class="custom-input pl-8 font-black text-gray-900" placeholder="0.00">
                            </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="duration_days" value="{{ old('duration_days', $package->duration_days) }}" required class="custom-input font-bold" placeholder="Total duration">
                        ...
                        <div>
                            ...
                            <input type="text" name="location" value="{{ old('location', $package->location) }}" required class="custom-input" placeholder="e.g. Dubai, UAE">
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" value="{{ old('start_date', optional($package->start_date)->format('Y-m-d')) }}" class="custom-input">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-card p-6">
                    ...
                    <div class="space-y-6">
                        <div x-data="fileUploader">
                            <label class="input-label">Cover Thumbnail</label>
                        ...
                        <div x-data="fileUploader">
                            ...
                            @if($package->images && count($package->images) > 0)
                                <div class="flex flex-wrap gap-3 mb-4">
                                    @foreach($package->images as $index => $img)
                                        <div class="relative">
                                            <img src="{{ filter_var($img, FILTER_VALIDATE_URL) ? $img : Storage::url($img) }}" class="h-20 w-20 object-cover rounded-xl border border-gray-100 shadow-sm">
                                            <button type="button" 
                                                onclick="if(confirm('Delete this image?')) { document.getElementById('delete-image-{{ $index }}').submit(); }"
                                                class="absolute -top-2 -right-2 w-6 h-6 bg-white border border-gray-200 rounded-full text-red-500 shadow-sm flex items-center justify-center hover:bg-red-50 transition">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"/></svg>
                                            </button>
                                        </div>
                            ...
                            <div x-show="fileName" class="mt-2 p-2 bg-blue-50 rounded-lg border border-blue-100 flex items-center justify-between gap-2">
                                <div class="flex items-center gap-2 min-w-0">
                                    <div class="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div>
                                    <span class="text-[9px] font-bold text-blue-700 uppercase tracking-tighter truncate" x-text="fileName"></span>
                                </div>
                                <button type="button" @click="clearSelection('images-input')" class="text-blue-500 hover:text-red-500"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"/></svg></button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {{-- Key Attributes --}}
                @php
                    $travelDataItems = old('travel_data', $package->travel_data ?? []);
                    if (empty($travelDataItems) || !is_array($travelDataItems)) {
                        $travelDataItems = [['label' => '', 'content' => '']];
                    }
                @endphp
                <div class="form-card p-6" x-data="{ items: {{ json_encode(array_values($travelDataItems)) }} }">
