<x-admin-layout>
    ...
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold" style="color:#0F1419;">Edit Package</h2>
        <p class="text-sm mt-0.5 truncate" style="color:#6B7280;">{{ $package->title }}</p>
    </div>
            ...
            <div class="flex-1 min-w-0 space-y-6">
                ...
                <div class="rounded-xl border p-6 space-y-5" style="background:#fff;border-color:#E6E8EC;">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="location" value="{{ old('location', $package->location) }}" required
                                   class="w-full px-3 py-2.5 rounded-lg border text-sm transition focus:outline-none focus:border-[#C8102E]"
                                   style="border-color:#E6E8EC;color:#0F1419;">
                            @error('location') <p class="text-xs mt-1" style="color:#C8102E;">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold tracking-widest uppercase mb-1.5" style="color:#6B7280;">Start Date</label>
                            <input type="date" name="start_date" value="{{ old('start_date', optional($package->start_date)->format('Y-m-d')) }}"
                                   class="w-full px-3 py-2.5 rounded-lg border text-sm transition focus:outline-none focus:border-[#C8102E]"
                                   style="border-color:#E6E8EC;color:#0F1419;">
                ...
                <div class="rounded-xl border p-6 space-y-5" style="background:#fff;border-color:#E6E8EC;">
                    ...
                    <div>
                        ...
                        <textarea name="{{ $field['name'] }}" rows="{{ $field['name'] === 'description' ? 6 : 4 }}"
                                  ...
                                  style="border-color:#E6E8EC;color:#0F1419;">{{ old($field['name'], $package->{$field['name']}) }}</textarea>
                    ...
                    @endphp
                    <div x-data="{ items: {{ json_encode(array_values($travelDataItems)) }} }">
                    ...
                    @endphp
                    <div x-data="{ days: {{ json_encode(array_values($itineraryData)) }} }">
                        ...
                        <div x-data="{ items: {{ json_encode(old('inclusions', $package->inclusions ?? [''])) }} }">
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <div class="flex gap-2 mb-2">
                                    ...
                                    <button type="button" @click="items.splice(index, 1)" x-show="items.length > 0"
                ...
                <div class="rounded-xl border p-6 space-y-5" style="background:#fff;border-color:#E6E8EC;">
                    ...
                    <div x-data="fileUploader">
                        <label class="block text-[10px] font-bold tracking-widest uppercase mb-1.5" style="color:#6B7280;">Thumbnail Image</label>
                        @if($package->thumbnail)
                            <img src="{{ Storage::url($package->thumbnail) }}"
                                 alt="Current Thumbnail"
                                 class="h-28 w-44 object-cover rounded-xl mb-3 shadow-sm"
                        ...
                        <input type="file" name="thumbnail" @change="handleFileChange" accept="image/*"
                               ...
                               style="color:#6B7280;">
                        <p class="text-xs mt-1" style="color:#6B7280;">Leave blank to keep the current thumbnail.</p>
                    ...
                    {{-- Gallery --}}
                    <div x-data="fileUploader">
                        <label class="block text-[10px] font-bold tracking-widest uppercase mb-1.5" style="color:#6B7280;">Gallery Images</label>
                        ...
                        <input type="file" name="images[]" @change="handleFileChange" multiple accept="image/*"
                               class="w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:cursor-pointer"
                               style="color:#6B7280;">
                        <p class="text-xs mt-1" style="color:#6B7280;">Upload multiple images to append to the gallery.</p>
                        <div x-show="fileName" class="mt-1.5 text-xs font-medium" style="color:#065F46;display:none;">
                            Selected: <span x-text="fileName"></span> (<span x-text="fileSize"></span>)
                        </div>
            ...
            <div class="w-full lg:w-72 flex-shrink-0 space-y-4">
                <div class="rounded-xl border p-5 sticky top-6" style="background:#fff;border-color:#E6E8EC;">
                    ...
                    <div class="flex items-center gap-2 mb-4">
                        <span class="text-xs" style="color:#6B7280;">Status:</span>
                        @if($package->is_active)
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold" style="background:#D1FAE5;color:#065F46;">Active</span>
                        @else
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold" style="background:#F3F4F6;color:#6B7280;">Inactive</span>
                        @endif
                    </div>
                    ...
                    <a href="{{ route('packages.show', $package->slug) }}" target="_blank"
                       class="mt-3 w-full py-2 rounded-xl text-sm font-semibold text-center flex items-center justify-center gap-1.5 transition hover:opacity-70"
                       style="color:#18130E;border:1px solid #E6E8EC;">
                ...
                {{-- Danger Zone --}}
                <div class="rounded-xl border p-5" style="border-color:#FECACA;background:#FFF5F5;">
                    <h4 class="text-[10px] font-bold tracking-widest uppercase mb-3" style="color:#C8102E;">Danger Zone</h4>
                    <form action="{{ route('admin.packages.destroy', $package) }}" method="POST"
                          onsubmit="return confirm('Permanently delete this package? This cannot be undone.');">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                class="w-full py-2 rounded-lg text-sm font-semibold transition hover:opacity-80"
                                style="background:#FEE2E2;color:#C8102E;">
                            Delete Package
                        </button>
                    </form>
                </div>
